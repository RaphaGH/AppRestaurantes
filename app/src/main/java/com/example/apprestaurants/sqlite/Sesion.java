package com.example.apprestaurants.sqlite;
...
public class Sesion extends SQLiteOpenHelper {
    private static final String s_nombre_bd = "rest.db";
    private static final int i_version_bd = 1;
    private static final String s_tabla_usuario = "CREATE TABLE IF NOT EXISTS USUARIO (ID INTEGER, CORREO VARCHAR(50), CLAVE VARCHAR(50))";
    //constructor de la base de datos interna
    ...
    public boolean actualizar_dato_usario(int i_id, String s_campo, String s_valor){
        boolean b_ret = false;
        SQLiteDatabase db = getWritableDatabase();

        if(db!=null){
            db.execSQL("UPDATE USUARIO SET "+s_campo+" WHERE ID="+i_id);
            db.close();
            b_ret = true;
        }
        return b_ret;
